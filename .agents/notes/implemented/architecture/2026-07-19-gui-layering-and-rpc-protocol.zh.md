...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）通过回环 HTTP／WebSocket 载体托管 Web profile 并在 Electron 中呈现；它的进程与安全边界归[桌面外壳决策](2026-08-16-codepilot-desktop-shell.md)所有。IPC fetch 载体仍是一种不会改变协议模型的替代方案。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器和当前回环 Electron 壳；零 workspace 依赖（注册表经结构注入到达）；IPC 壳可以替换它 |
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | bin 粗分发、vite Web 入口，以及围绕 Web profile 的 Electron 托管进程 | 应用不增加协议方法；dist 定位、进程恢复和安装包等 workspace 与交付知识留在 app |
...
现有三个应用保持这一区分：Web 应用挂载 Host、载体与浏览器组合；桌面应用托管同一套回环组合；`dsh --profile headless` 则挂载直接使用核心服务的 runner，不包含 Host、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。
...
| IPC 桥子类（假想方案；当前 Electron 壳使用 `WebApiClient`） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |

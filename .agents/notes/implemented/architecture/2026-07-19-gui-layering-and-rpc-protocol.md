...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) supervises the Web profile through its loopback HTTP/WebSocket carrier and renders it in Electron; the [desktop-shell decision](2026-08-16-codepilot-desktop-shell.md) owns its process and security boundary. An IPC fetch carrier remains an alternative that does not change the protocol model.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser and the current loopback Electron shell; zero workspace dependencies (the registry arrives by structural injection); an IPC shell may replace it |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch, the vite Web entry, and the Electron supervisor around the Web profile | Applications do not add protocol methods; workspace and delivery knowledge such as dist location, process recovery, and installers stays in the app |
...
The three existing applications preserve the division: the Web application mounts Host, carrier, and browser composition; the desktop application supervises that same loopback composition; and `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (hypothetical; the current Electron shell uses `WebApiClient`) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |

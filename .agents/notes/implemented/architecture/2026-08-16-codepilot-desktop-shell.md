...
Official packages are produced only by the `Desktop` GitHub Actions workflow. A manual dispatch may retain ad-hoc native packages for seven days as Actions artifacts, but it cannot update Releases. A version-matched `v*` tag publishes the native macOS, Windows, and Linux installers together with prebuilt tarballs for the CodePilot theme, Worktree Files, Schedule summary, and Session-log export plugins, plus one checksum manifest. Each plugin tarball declares `dsh.bundle.patch` and carries its own `cordis.patch.yml`, so a local Web profile installs it through one `dsh plugin --profile web add <release-url>` command rather than a source checkout or handwritten patch. CI installs all four archives into an empty Web profile and verifies their composed row ids before publishing them. Manual macOS artifacts use electron-builder's explicit ad-hoc identity and strict bundle verification; tagged macOS artifacts instead require a Developer ID Application certificate and the configured Team ID, so an absent or mismatched release identity stops publication. The root README therefore leads desktop users to Releases and keeps source-build instructions in its Development section without presenting local packaging as a release path.
...
The app depends on the Web profile remaining embeddable on loopback and on its settled URL log line. The theme plugin requires visual verification after upstream token or sidebar changes. Native dependencies require installers to be built on their target operating system. Developer ID signing lets macOS users approve an unnotarized preview through Privacy & Security instead of removing quarantine metadata; notarization remains necessary to remove that first-launch approval.

...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/workspace/ui-worktree': { kind: 'indirect', reason: 'The draft action inserts only unsent human input; conversation submission owns the model-visible message.' },
  'packages/schedule/ui-schedule-summary': { kind: 'none', reason: 'The read-only presentation plugin inspects persisted Schedule state and contributes no prompt, tool, event, or request.' },
  ...
  'packages/client/ui-codepilot-theme': { kind: 'none', reason: 'Browser-side visual theme plugin; registers nothing model-facing.' },

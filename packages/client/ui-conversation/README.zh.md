...
会话领域：骨架（标题栏／标签页／编辑器／空状态）、聊天视图（分组步骤摘要流、流式尾部隔离与轮次状态）、编辑器上下文详情（会话统计与上下文占用率）、输入区 dock（队列行加 todo 计划条）、详情壳层，以及按 scope 寻址的 ConversationController。工具展示属于 [`ui-tool`](../ui-tool/README.md)。
...
聊天统计的 token 账目来自经标准套件 `useProjection` 读取的通用 token-meter 投影 `tokenUsage`：计费输入为未缓存输入、缓存读取与缓存写入之和；缓存命中率以缓存读取除以该总量。轮次与步骤计数、LLM（大语言模型）与工具墙钟时间、以及延迟／吞吐分组来自全日志的 `sessionStats` 投影，因此分页与压缩不会改变这些数字。每个已结算轮次仍可在 assistant footer 中追加 hover 后显示的 timing 标签。上下文占用率渲染为 composer 尾部的 ContextMeter：它位于模型 seat 之后，始终可用，并接收可选的 `contextPressure` 投影。它会打开唯一承载 token、缓存、timing、计数与组成明细的面板，把「已用百分比」标题和 `~已用 / 容量` 数字，与来自 `contextBreakdown` 投影的分色分段进度条及启发式组成明细行（系统提示词、工具、对话消息）放在一起。未组合 token-meter 的部署会省略不可用的分组。占用率是刻意为之的近似值：分子与容量是两个相互独立的「后写覆盖」投影字段，并非同一次请求的原子观测（[原理](../../llm/token-meter/README.md)）。
...
- **统计数据的回退折算只覆盖窗口内消息流**：未组合 `sessionStats` 投影单元的装配中，上下文详情面板里的所有数字都由快照的 assistant `timing` 与工具 call/result 配对折算，落在已加载事件窗口之外的节点（更早的历史）不计入，数字随加载页数增长。

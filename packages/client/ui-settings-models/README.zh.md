...
模型设置与产品引导插件。同一个 Cordis 插件会注册服务商页面、模型页面、一项按 profile 隔离的引导设置，以及一个不绑定厂商的首次使用引导。只有当前没有任何可用服务商时才会显示引导；用户可以直接前往服务商页面，也可以跳过，因此浏览体验不会被阻断。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
引导步骤从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。没有可用服务商时，弹窗不会偏向 DeepSeek，也不会直接索要密钥：「添加服务商」会打开完整的服务商管理页面；选择「暂时跳过」后会在当前 Harness 设置 profile 中记录用户选择，切换工作区、刷新或以后重新启动应用都不会反复打扰，同时不会压掉另一个 profile 的首次引导；用户仍可随时从设置中添加服务商。由于服务商探测会异步完成，只要工作区目录浏览器等其他对话框已经打开，引导就会等到该流程关闭后再显示，绝不会替换用户正在进行的操作。联接失败或部署只读时，该步骤不渲染并直接完成；服务商页仍是诊断界面。

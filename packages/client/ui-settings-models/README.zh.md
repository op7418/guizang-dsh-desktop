...
模型设置与产品引导插件。同一个 client Cordis 插件会注册服务商、模型页面和一个不绑定厂商的首次使用引导。只有当前没有任何可用服务商时才会显示引导；用户可以直接前往服务商页面，也可以在当前空白会话中跳过，因此浏览体验不会被阻断。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。

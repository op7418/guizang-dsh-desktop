...
引导步骤从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。没有可用服务商时，弹窗不会偏向 DeepSeek，也不会直接索要密钥：「添加服务商」会打开完整的服务商管理页面，「暂时跳过」只完成协调器当前这一轮。由于服务商探测会异步完成，只要工作区目录浏览器等其他对话框已经打开，引导就会等到该流程关闭后再显示，绝不会替换用户正在进行的操作。联接失败或部署只读时，该步骤不渲染并直接完成；服务商页仍是诊断界面。

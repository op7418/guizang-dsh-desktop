...
原生目录选择界面：原生选取交互的浏览器半边。它通过 ui-workspace 的两个 directory-flow 洞（`conversation.hero.workspace.directoryFlow` 与 `sidebar.workspaces.directoryFlow`）装入一个无渲染占位者，并为每个 `open` 请求回报恰好一个结果——选中的路径、取消、或失败。桌面外壳可以通过隔离的 preload 暴露 `window.pilotHarness.pickDirectory()`；该占位者会优先使用这个由原生窗口拥有的选择框，不存在时再通过 `ctx.workspaces.pickDirectory()` 驱动本地 Host 的操作系统选择框。普通 Web 对话框属于 [`dsh-host-directory-picker-native`](../../host/directory-picker-native/README.md)；挂载本包即用一行 cordis.yml 把界面与该后端组合起来，而桌面环境无需改动 workspace owner 即可复用同一 UI 插件。
...
- **仅限本地显示载体** —— Web 选择框开在运行 Host 的机器上，嵌入式桌面选择框则开在 Electron 显示器上；远程浏览器部署需要改用 `-browse` 组合。平台失败通过 owner 的可重试文件夹对话框呈现。

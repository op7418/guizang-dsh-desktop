...
/** Replace only the host-owned revision query while preserving every other URL component. */
function withRevision(url: string, rev: string): string {
...
export class ClientModuleSystem implements ClientModuleLoader {
  ...
  invalidate(id: string, rev?: string): void {
    ...
    const row = this.graphRows.get(id)
    if (row !== undefined) this.graphRows.set(id, { ...row, rev, url: withRevision(row.url, rev) })
  }
}

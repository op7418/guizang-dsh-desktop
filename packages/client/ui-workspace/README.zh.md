...
Session 行保持紧凑、便于扫读：标题、相对时间以及等待／运行／完成状态留在行内；延迟出现的悬浮卡片展开完整标题、Workspace、目录、Agent preset 与状态。浏览器声明根作用域的 list 子 slot `sidebar.workspaces.session.detail`。领域插件通过 owner 提供的可序列化 class 与图标名贡献语义化的 `branch`、`model` 或 `reminder` 行，使间距、字体、颜色与截断继续服从 Workspace 的视觉 Token，同时不经 owner props 传递 React 内容。模型贡献直接读取会话列表携带的 `sessionModel` 投影，不会激活冷历史；Worktree 与 Schedule 贡献只在悬浮卡片挂载期间发起各自只读的 Host 查询。
...
- **可选详情行取决于插件能力**：投影建立前已经使用过的 Session、非 Git Workspace，或没有组合 Schedule 摘要插件的部署，会直接省略对应详情行。

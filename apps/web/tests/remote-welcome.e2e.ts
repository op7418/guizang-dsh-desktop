...
describe.skipIf(MODE === 'record')('web e2e: remote welcome notice', () => {
  ...
  it('advances process-locally and presents the notice again after reload', async () => {
    // Onboarding no longer covers the initial project picker: a cold client
    // chooses its workspace first, then sees the product notice.
    await connectFreshWorkspaceZh(page, scaffold.workspaceCwd, 'remote-welcome')
